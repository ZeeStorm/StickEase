...
class CI_Form_validation {
	...
	function run($data, $group = '')
	{
		...
		// corresponding $this->_data item and test for errors
		foreach ($this->_field_data as $field => $row)
		{
			// Fetch the data from the corresponding $this->_data array and cache it in the _field_data array.
	...
	/**
	 * Traverse a multidimensional $this->_data array index until the data is found
	 *
	 * @access	private
	 * @param	array
	 * @param	array
		...
		foreach ($this->_field_data as $field => $row)
		{
			if ( ! is_null($row['postdata']))
			{
				if ($row['is_array'] == FALSE)
				{
					...
					{
						$this->_data[$row['field']] = $this->prep_for_form($row['postdata']);
					}
				}
				else
				{
	...
	{
		// If the $this->_data data is an array we will run a recursive call
		...
		if (is_null($postdata) AND $callback == FALSE)
		{
			if (in_array('isset', $rules, TRUE) OR in_array('required', $rules))
			{
				...
				// if array, execute on each thing and throw it back
				if ( is_array( $line ) )
				{
					$message = array();
					...
					{
						array_push( $message, sprintf($chunk, $this->_translate_fieldname($row['label'])));
					}
				}
				else
				{
		...
		foreach ($rules As $rule)
		{
			...
			if ($result === FALSE)
			{
				...
				if ( is_array( $line ) )
				{
					...
					foreach ( $line as $chunk )
					{
						array_push( $message, sprintf($chunk, $this->_translate_fieldname($row['label']), $param));
					}
				}
				else
				{
	...
	function domain($str)
	{
		return ( !preg_match("/^([a-z0-9\-\.]+)\.([a-z]+)$/i", $str)) ? FALSE : TRUE;
	}
